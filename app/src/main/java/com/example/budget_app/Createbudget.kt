...
class Createbudget : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        navView.setNavigationItemSelectedListener { menuItem ->
            when (menuItem.itemId) {
                ...
                R.id.nav_categories -> { startActivity(Intent(this, Category::class.java)); finish() }
    ...
    private fun setupDropdowns() {
        // 1. Set default categories IMMEDIATELY so the dropdown isn't dead while waiting for Firebase
        ...
        // 2. Load custom categories in the background
        val userId = auth.currentUser?.uid ?: return
        val customRef = database.getReference("users").child(userId).child("custom_categories")
        customRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val newAdapter = ArrayAdapter(this@Createbudget, android.R.layout.simple_spinner_dropdown_item, updatedCategories)
                etBudgetName.setAdapter(newAdapter)
            }
            override fun onCancelled(error: DatabaseError) {}
        })
        
        // 3. Force dropdown to show on first tap
        etBudgetName.setOnTouchListener { v, _ ->
        ...
        spinnerPeriod.setOnTouchListener { v, _ ->
    ...
    private fun fetchUserProfile() {
        ...
        database.getReference("users").child(user.uid).child("profile")
        ...
        transRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (data in snapshot.children) {
                    ...
                    if (trans != null) transactionList.add(trans)
                }
                updateSpentMap()
            }
            override fun onCancelled(error: DatabaseError) {}
    ...
    private fun saveBudget() {
        ...
        val budgetRef = if (existingBudget != null) {
            database.getReference("users").child(userId).child("budgets").child(existingBudget.budgetId)
        } else {
            database.getReference("users").child(userId).child("budgets").push()
        }
